...
let questions = [
  ...
  {
    question:
      "Q#7 Which of the Seven Wonders of the World is from the following",
    choices: [
      "A. Colosseum",
...
for (let i = 0; i < questions.length; i++) {
  console.log(
    chalk.blue(
      `${capitalizedName}, ${urduCounting[i]} prashn apki screen par yeh raha`
    )
  );
  ...
  if (input.userAnswer === questions[i].answer) {
    ...
    console.log(
      chalk.red("Incorrect. The correct answer is: " + questions[i].answer)
    );
  }
}
